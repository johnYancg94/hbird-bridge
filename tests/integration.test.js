...
assert(
    manifest.includes('<Extension Id="com.hbird.bridge.ps.panel" Version="1.5.0"/>'),
...
assert(main.includes('saveSettings();'), 'legacy settings migration must persist the new settings file');
...
assert(installGuide.includes('智能对象导入：替换当前图层并保持原位置'), 'install guide must document the current smart import');
...
const browseButtonPosition = index.indexOf('id="browseDirBtn"');
const openFolderButtonPosition = index.indexOf('id="openAssetsDirBtn"');
const scanButtonPosition = index.indexOf('id="scanFolderBtn"');
...
assert(openFolderButtonPosition > browseButtonPosition, 'open-folder button must follow browse button');
assert(archiveButtonPosition > scanButtonPosition, 'archive button must follow scan button');
assert(archiveButtonPosition < autoRefreshPosition, 'archive button must precede auto-refresh status');
assert(index.includes('title="打开当前素材目录"'), 'open-folder button needs an explanatory title');
assert(index.includes('aria-label="打开当前素材目录"'), 'open-folder button needs an accessible label');
assert(style.includes('.btn-icon'), 'compact open-folder button styling must exist');
...
assert(index.includes('title="替换当前图层并保持原位置"'), 'smart-object import needs an explanatory title');
assert(
...
assert(main.includes("elements.openAssetsDirBtn.addEventListener('click', openAssetsDirectory)"));
...
assert(
    !smartObjectBranch.includes('doc.activeLayer.rasterize(RasterizeType.ENTIRELAYER)'),
    'smart-object import must remain independent from normal layer rasterization'
);
...
assert(
    main.includes('const explorerDirectory = path.win32.normalize(CONFIG.assetsDir);'),
...
assert(main.includes('function buildArchiveMovePlan'));
